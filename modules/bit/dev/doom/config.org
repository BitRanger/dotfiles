...
(doom! :input
       ...
       (nix +lsp +fmt +tree-sitter)               ; I hereby declare "nix geht mehr!"
       ;;ocaml             ; an objective camel
       ;;odin              ; C, minus its footguns
       (org +dragndrop +gnuplot +hugo +crypt +journal +jupyter +noter +pandoc +pomodoro +present +pretty +roam)               ; organize your plain life in plain text
...
;; accept. For example:
;;
(setq doom-font (font-spec :family "Iosevka Nerd Font" :size 14 :weight 'semi-light)
...
;; Enable org-mode logging the time that tasks are marked done at
(setq org-log-done 'time) ;; can also set to 'note to be prompted to write a note when marking task as done
...
(set-frame-parameter nil 'alpha-background 80)              ; make current frame transparent
(add-to-list 'default-frame-alist '(alpha-background . 80)) ; make new frames transparent
...
(defcustom transparency-percentage 80
  "Emacs GUI Transparency Percentage"
  :set (lambda (sym val)
         (set-frame-parameter nil 'alpha-background (sym val))              ; make current frame transparent
(add-to-list 'default-frame-alist '(alpha-background . (sym val))) ; make new frames transparent
         )
  )
#+end_src
* packages.el
